...
**Gate question:** Is an AI agent (Gemini CLI, Codex CLI, Cline) invoked in a workflow where external users can influence the prompt?
...
# INSTALLATION (Codex CLI Skill)

To use this as a Codex CLI skill, copy this file to your skills directory:

```bash
# Option A: Clone the repo and link the skill
git clone https://github.com/shuvonsec/claude-bug-bounty.git ~/.agents/skills/bug-bounty
ln -s ~/.agents/skills/bug-bounty/SKILL.md ~/.agents/skills/bug-bounty/SKILL.md

# Option B: Direct copy
mkdir -p ~/.agents/skills/bug-bounty
curl -s https://raw.githubusercontent.com/shuvonsec/claude-bug-bounty/main/SKILL.md \
  -o ~/.agents/skills/bug-bounty/SKILL.md
```

Then in Codex CLI, this skill loads automatically when you ask about bug bounty, recon, or vulnerability hunting.
