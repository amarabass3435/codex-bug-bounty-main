...
- `CODEX.md`: Skills count 8 → 9, added `meme-coin-audit` to skill table; Commands 13 → 14, added `$token-scan`
...
- `CODEX.md`: Skills count 7 → 8, added `bb-methodology` to skill table
...
Major restructure into a full Codex CLI plugin with multi-component architecture.
...
- `agents/` directory with 5 specialized agents
  - `recon-agent` — runs recon pipeline (model unset)
  - `report-writer` — generates reports (model unset)
  - `validator` — validates findings (model unset)
  - `web3-auditor` — audits contracts (model unset)
  - `chain-builder` — builds exploit chains (model unset)
- `.codex/hooks.json` — session start/stop hooks with hunt reminders
- `rules/hunting.md` — 17 critical hunting rules (always active)
- `rules/reporting.md` — 12 report quality rules (always active)
- `CODEX.md` — plugin overview and quick-start guide
