<p align="center">
  <img src="logo.png" alt="Codex Bug Bounty Logo" width="320"/>
</p>
...
_Your AI hunting partner that remembers past targets, spots vulnerabilities, and writes reports for you._
...
[![Codex CLI](https://img.shields.io/badge/Codex_CLI-Plugin-0A7FE7.svg?style=flat-square)](#-what-is-this)
...
**This tool** is a plugin for Codex CLI that turns it into a professional bug bounty hunting partner. Instead of juggling 15 different tools and writing reports from scratch, you just type a command and the AI handles the rest.
...
> **Prerequisite:** You need Codex CLI installed. It runs in your terminal.
...
chmod +x install.sh && ./install.sh               # installs AI skills + commands into Codex CLI
```

**Step 2 — Start hunting**

```bash
codex                           # open Codex CLI in your terminal
...
> **Don't use Codex CLI?** Run the Python tools directly:
>
> ```bash
...
A team of specialists, each doing one job. Codex orchestrates; memory persists across sessions.

```mermaid
flowchart TD
    You([You]) --> Codex[Codex CLI]
    Burp[Burp MCP<br/>browser traffic] -.-> Codex
    H1[HackerOne MCP<br/>program intel] -.-> Codex
    Codex --> Recon[Recon<br/>subdomains · URLs · APIs]
    Codex --> Hunt[Hunt<br/>test &amp; validate]
    Codex --> Report[Report<br/>H1 · Bugcrowd · Immunefi]
    Recon --> Memory[(Hunt Memory<br/>persistent across sessions)]
    Hunt --> Memory
    Report --> Memory
    Memory -.feeds back.-> Codex
```
...
- **`$memory-gc`** — new command to inspect, force-rotate, or purge backup files in the hunt-memory tree.
...
You also need Codex CLI installed.
...
chmod +x install.sh && ./install.sh               # AI skills + commands into Codex CLI
```
...
| **[public-skills-builder](https://github.com/shuvonsec/public-skills-builder)**                         | Turns 500+ public bug writeups into Codex skill files           |
