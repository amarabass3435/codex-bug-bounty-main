...
Codex CLI is a command-line tool that runs in your terminal and lets you use AI to write code, analyze files, and run tasks. This plugin turns it into a bug bounty hunting assistant.
...
Every time you finish a hunt session, the tool automatically saves a summary of what you tested and found. This gets stored locally in your Codex project directory under `hunt-memory/`.
...
This plugin can connect to Burp via MCP (an integration protocol) so Codex can read your proxy history and replay requests through Burp. **But Burp is optional.** Every command works without it — the tool falls back to using `curl` for HTTP requests automatically.
...
- Hunt memory is stored in `hunt-memory/` inside your Codex project directory
- Audit logs stay in the same directory
- Nothing is sent to any server outside of your normal Codex CLI usage
