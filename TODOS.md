...
**What:** Auth credentials (API keys, cookies, Bearer tokens) passed to `$hunt` or `$autopilot` via Bash env vars or direct input persist in the Codex CLI conversation transcript. Anyone with access to the Codex project directory can read them.
