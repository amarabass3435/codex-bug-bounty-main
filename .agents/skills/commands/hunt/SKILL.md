---
name: hunt
description: Start hunting on a target — loads scope, reads disclosed reports, picks best attack surface based on tech stack, runs targeted vuln checks.
---
...
**One session per target.** Codex accumulates context — testing two targets in one session
causes cross-contamination where payloads, assumptions, and findings from target A
affect target B.

```bash
codex  ->  $hunt targetA.com   # Terminal 1
codex  ->  $hunt targetB.com   # Terminal 2 (separate process)
```
...
Requires Chrome MCP configured in Codex `config.toml`. Enables:
...
If the agent gives you a generic message like "try testing for XSS" or "check for IDOR",
