---
name: memory-gc
description: Inspect or rotate hunt-memory JSONL files (audit.jsonl, patterns.jsonl, journal.jsonl). Caps file size and keeps rotated backups so memory does not grow unbounded.
---
...
2. **On session end** — a `Stop` hook in `.codex/hooks.json` runs `python3 -m tools.memory_gc --rotate` so long sessions that wrote a lot but never crossed the cap mid-session still get cleaned up.

So this command is mainly for ad-hoc reporting (`$memory-gc` with no args) and manual cleanup of accumulated backups (`$memory-gc --purge-backups`).
