---
name: autopilot
description: Run autonomous hunt loop on a target — scope check -> recon -> rank surface -> hunt -> validate -> report with configurable checkpoints.
---
...
**Start a fresh Codex session per target.** Codex accumulates context across a session —
...
```bash
# Terminal 1: target A
codex  ->  $autopilot targetA.com

# Terminal 2: target B (separate process)
codex  ->  $autopilot targetB.com
```
